...
'''
Contains augmentation for to be applied to simulated images to make the distribution and texture more 
similar to what actual images are 
...
def transporter_aug_train(mgam1 = [2, 5], mgam2=[20, 70], cl = [0.35, 0.8], agamma1= [0.2, 0.6], agamma2=[10,50]):
    def augmentation(img):
        arr = img.numpy()
        arr = (arr-arr.min())/(arr.max()- arr.min())
        mgam1_val = sample_gaussian_in_range(np.array(mgam1).mean(), 1.5, mgam1[0], mgam1[-1])
